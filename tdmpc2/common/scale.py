...
class RunningScale:
    ...
    def __init__(self, cfg):
        self.cfg = cfg
        self._value = torch.ones(1, dtype=torch.float32, device=torch.device(self.cfg.device))
        self._percentiles = torch.tensor([5, 95], dtype=torch.float32, device=torch.device(self.cfg.device))
