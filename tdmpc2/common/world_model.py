...
class OCRewardModel(nn.Module):
    def __init__(self, cfg):
        ...
        self.gnn = layers.GNN(input_dim=self.cfg.slot_dim, hidden_dim=self.cfg.latent_dim,
                              action_dim=self.cfg.action_dim, num_objects=self.cfg.n_slots, ignore_action=False,
                              copy_action=True, edge_actions=True)
        self.mlp = nn.Linear(in_features=self.cfg.slot_dim, out_features=max(self.cfg.num_bins, 1))
...
class OCPolicy(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.act = nn.ReLU(inplace=True)
        self.gnn = layers.GNN(input_dim=self.cfg.slot_dim, hidden_dim=self.cfg.latent_dim, action_dim=0,
                              num_objects=self.cfg.n_slots, ignore_action=True, copy_action=False, edge_actions=False)
        self.mlp = nn.Linear(in_features=self.cfg.slot_dim, out_features=2 * self.cfg.action_dim)
