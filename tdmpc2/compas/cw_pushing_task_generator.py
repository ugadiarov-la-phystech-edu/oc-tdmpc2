...
from compas.config_pushing import Difficulty
...
class PushingTaskGenerator(BaseTask):
    ...
    def __init__(self,
        ...
        self._collision_radius = 0.095

        if isinstance(tool_block_color, str):
            tool_block_color = colors.to_rgb(tool_block_color)

        if isinstance(goal_block_color, str):
            goal_block_color = colors.to_rgb(goal_block_color)
    ...
    def _pair_has_collision(self, p1, p2):
        ...
        return dist <= self._collision_radius
    ...
    def _set_up_stage_arena(self):
        ...
        creation_dict = {
            ...
            'color': self._task_params['tool_block_color']
        }
        ...
        creation_dict = {
            ...
            'color': self._task_params['goal_block_color']
        }
    ...
    def _sample_new_goal_easy(self):
        return {}

    def _sample_new_goal_medium(self):
        ...
        pos_low_bound[-1] = self._stage.get_object_state('tool_block',
                                                         'size')[-1] / 2.0
        pos_upper_bound = np.array(
            intervention_space['goal_block']['cylindrical_position'][1])
        pos_upper_bound[-1] = self._stage.get_object_state(
            'tool_block', 'size')[-1] / 2.0
    ...
    def _sample_new_goal_hard(self):
        intervention_space = self.get_variable_space_used()
        ...
        # goal_pos_low_bound = np.array(
        #     intervention_space['goal_block']['cylindrical_position'][0])
        # goal_pos_low_bound[-1] = self._stage.get_object_state('tool_block',
        #                                                  'size')[-1] / 2.0
        # goal_pos_upper_bound = np.array(
        #     intervention_space['goal_block']['cylindrical_position'][1])
        # goal_pos_upper_bound[-1] = self._stage.get_object_state(
        #     'tool_block', 'size')[-1] / 2.0

        # tool_pos_low_bound = np.array(
        #     intervention_space['tool_block']['cylindrical_position'][0])
        # tool_pos_low_bound[-1] = self._stage.get_object_state('goal_block',
        #                                                  'size')[-1] / 2.0
        # tool_pos_upper_bound = np.array(
        #     intervention_space['tool_block']['cylindrical_position'][1])
        # tool_pos_upper_bound[-1] = self._stage.get_object_state(
        #     'goal_block', 'size')[-1] / 2.0

        tool_cyl_pos = np.random.uniform(*intervention_space['tool_block']['cylindrical_position'])

        goal_cyl_pos = np.random.uniform(*intervention_space['goal_block']['cylindrical_position'])

        while self._pair_has_collision(cyl2cart(tool_cyl_pos),
                                       cyl2cart(goal_cyl_pos)):
            goal_cyl_pos = np.random.uniform(*intervention_space['goal_block']['cylindrical_position'])

        interventions['goal_block']['cylindrical_position'] = goal_cyl_pos
        interventions['goal_block']['euler_orientation'] = np.random.uniform(
            intervention_space['goal_block']['euler_orientation'][0],
            intervention_space['goal_block']['euler_orientation'][1])
        interventions['tool_block']['cylindrical_position'] = tool_cyl_pos
        interventions['tool_block']['euler_orientation'] = np.random.uniform(
            intervention_space['tool_block']['euler_orientation'][0],
            intervention_space['tool_block']['euler_orientation'][1])
    ...
    def sample_new_goal(self, level=None):
        ...
        if self.difficulty == Difficulty.EASY:
            return self._sample_new_goal_easy()
        elif self.difficulty == Difficulty.MEDIUM:
            return self._sample_new_goal_medium()
        elif self.difficulty == Difficulty.HARD:
            return self._sample_new_goal_hard()
    ...
    def _calculate_dense_rewards(self, desired_goal, achieved_goal):
        ...
        goal_position = self._stage.get_object_state('goal_block',
                                                     'cartesian_position')
        goal_orientation = self._stage.get_object_state(
        ...
        if 'goal_block' in interventions_dict:
            if 'size' in interventions_dict['goal_block']:
                ...
                if 'cartesian_position' not in interventions_dict['tool_block'] and \
                    ...
                    cyl_pos_tool[-1] = interventions_dict['goal_block'][
                                           'size'][-1] / 2.0
                    interventions_dict['tool_block'][
                        'cylindrical_position'] = cyl_pos_tool
                if 'cartesian_position' not in interventions_dict['goal_block'] and \
                    ...
                    cyl_pos_goal[-1] = interventions_dict['goal_block'][
                                           'size'][-1] / 2.0
                    interventions_dict['goal_block'][
                        'cylindrical_position'] = cyl_pos_goal
            ...
            if 'size' in interventions_dict['tool_block']:
                ...
                if 'cartesian_position' not in interventions_dict['tool_block'] and \
                    ...
                    cyl_pos_tool = self._stage.get_object_state(
                        'tool_block', 'cylindrical_position')
                    cyl_pos_tool[-1] = interventions_dict['tool_block'][
                                           'size'][-1] / 2.0
                    interventions_dict['tool_block'][
                        'cylindrical_position'] = cyl_pos_tool
                if 'cartesian_position' not in interventions_dict['goal_block'] and \
                    ...
                    cyl_pos_goal = self._stage.get_object_state(
                        'goal_block', 'cylindrical_position')
                    cyl_pos_goal[-1] = interventions_dict['tool_block'][
                                           'size'][-1] / 2.0
                    interventions_dict['goal_block'][
                        'cylindrical_position'] = cyl_pos_goal
    ...
    def _adjust_variable_spaces_after_intervention(self, interventions_dict):
        spaces = [
            self._intervention_space_a, self._intervention_space_b,
