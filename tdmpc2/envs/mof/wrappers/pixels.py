...
class Pixels(gym.Wrapper):
    def __init__(self, env: gym.Env, image_size: Union[Tuple[int, int], int]) -> None:
        super().__init__(env)
        self.image_size = (image_size, image_size) if isinstance(image_size, int) else image_size
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(3,) + self.image_size, dtype=float)
