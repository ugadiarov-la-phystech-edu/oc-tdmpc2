...
class IsaacPandaPushWrapper(Env):
    metadata = {"render.modes": ["rgb_array"]}

    def __init__(self, isaac_panda_push_goal_wrapper):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=255, shape=(*observation_space.shape[2:], observation_space.shape[1]), dtype=np.uint8)
        action_space = self.env.action_space
        self.action_space = gym.spaces.Box(low=action_space.low, high=action_space.high, shape=action_space.shape, dtype=action_space.dtype)
    ...
    def step(self, action):
        observation, reward, done, info = self.env.step(action[np.newaxis])
        self._last_frame = self._get_image_from_obs_dict(observation)
        reward = reward[0]
        info = info[0]
        info['img'] = info.pop('image')
        info['goal_img'] = info.pop('goal_image')
